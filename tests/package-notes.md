...
Root cause: `decodeAudioData` on the WHOLE file fails in Chrome (decode
...
4. Fetch `bytes=startByte-endByte` with 5 s margins (RANGE_MARGIN_SECONDS),
   ...
   re-syncs past gaps — Quran recitation files embed ID3v2 tags at EVERY verse
   boundary inside the audio stream!), decode the slice (~1 MB, fast), return
   `{buffer, offsetSeconds}` where offsetSeconds accounts for the 5 s margin
   and the ID3 offset.
...
- `tests/real-export.mjs` — real API export. Env: SURAH, FROM, TO, RECITER
  (label match). Verified: surah 2 verses 1-5 with default (khalil_al_husary,
  95 MB) and mishari (116 MB) both export with avc1+Opus tracks, no audio
  warning. Playwright + system Chrome.
- `tests/export-loop.mjs` — mock-based loop; 5/5 pass; validates MP4 boxes
  (stsd fourcc at offset+20: avc1 + Opus).
...
- `tests/probe-slice.mjs` — replays the exact browser range logic against the
  real 116 MB file (total 116226409, id3Skip 45, sync 45, trimmed 1374356 →
  decodes OK, 85.87 s).
